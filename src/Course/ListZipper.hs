...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft p l = case moveLeft l of
  IsNotZ -> IsNotZ
  IsZ (ListZipper ls x rs) -> if p x
   then IsZ (ListZipper ls x rs)
   else findLeft p (ListZipper ls x rs)
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight p l = case moveRight l of
  IsNotZ -> IsNotZ
  IsZ (ListZipper ls x rs) -> if p x
    then IsZ (ListZipper ls x rs)
    else findRight p (ListZipper ls x rs)
...
moveLeftLoop (ListZipper Nil x r) =
  case reverse (x:.r) of
    ...
    Nil    -> ListZipper Nil x r -- unreachable code
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper ls x rs) =
  case rs of
    Nil -> case reverse (x:.ls) of
      x':.r' -> ListZipper Nil x' r'
      Nil    -> ListZipper ls x rs -- unreachable code
    r:.rs' ->
...
moveLeft ::
  ListZipper a
  -> MaybeListZipper a
moveLeft (ListZipper ls x rs) =
  case ls of
    Nil -> IsNotZ
    l:.ls' -> IsZ $ ListZipper ls' l (x:.rs)
...
moveRight ::
  ListZipper a
  -> MaybeListZipper a
moveRight (ListZipper ls x rs) =
  case rs of
    Nil -> IsNotZ
    r:.rs' -> IsZ $ ListZipper (x:.ls) r rs'
...
swapLeft ::
  ListZipper a
  -> MaybeListZipper a
swapLeft (ListZipper ls x rs) =
  case ls of
    Nil -> IsNotZ
    l:.ls' -> IsZ $ ListZipper (x:.ls') l rs
...
swapRight ::
  ListZipper a
  -> MaybeListZipper a
swapRight (ListZipper ls x rs) =
  case rs of
    Nil -> IsNotZ
    r:.rs' -> IsZ $ ListZipper ls r (x:.rs')
...
moveLeftN n l =
  case n of
    ...
    -1 -> moveRight l
    _ -> if n > 0
      then moveLeftN (n-1) -<< moveLeft l
      else moveLeftN (n+1) -<< moveRight l
...
moveLeftN' n (ListZipper l x r) =
  case moveLeftN n $ ListZipper l x r of
...
nth n (ListZipper l x r) =
  moveLeftN (length l - n) (ListZipper l x r)
...
-- prop> \lz -> rights (end lz) == Nil
  ...
  case moveRightN (length $ rights z) z of
    IsZ z' -> z'
    IsNotZ -> z
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft y (ListZipper ls x rs) =
  ListZipper (x:.ls) y rs
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight y (ListZipper ls x rs) =
  ListZipper ls y (x:.rs)
...
-- /Tip:/ Use `zipWith`
  (ListZipper fl fx fr) <*> (ListZipper l x r) =
    ListZipper (zipWith id fl l) (fx x) (zipWith id fr r)
...
instance Extend ListZipper where
  f <<= z =
    let dup = ((\x -> (f x, x)) <$>) . toOptional
    in ListZipper (unfoldr (dup . moveLeft) z) (f z) (unfoldr (dup . moveRight) z)
