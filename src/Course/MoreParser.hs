...
hex ::
  Parser Char
hex =
  thisMany 4 (satisfy isHexDigit) >>= \cs ->
  case readHex cs of
...
satisfyAll fs =
  character >>= \c ->
  if and $ sequence fs c then pure c else constantParser $ UnexpectedChar c
...
satisfyAny fs =
  character >>= \c ->
  if or $ sequence fs c then pure c else constantParser $ UnexpectedChar c
