...
satisfy f =
  ...
  if f c then valueParser c else unexpectedCharParser c)
...
sequenceParser (p:.ps) =
  flbindParser p (\c ->
  flbindParser (sequenceParser ps) (\cs ->
  valueParser (c:.cs)))
...
personParser =
  ...
  spaces1 >>> flbindParser phoneParser (\phone' ->
  valueParser (Person {age = age', firstName = firstName', surname = surname',
                       smoker = smoker', phone = phone'}))))))
